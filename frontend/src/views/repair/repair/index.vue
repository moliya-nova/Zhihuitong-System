<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="repairList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="处理结果" align="center" prop="handleContent" />
      <el-table-column label="处理时间" align="center" prop="handleTime" width="180">
        <template #default="scope">
          <span>{{ parseTime(scope.row.handleTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="报修照片" align="center" prop="repairImage" width="100">
        <template #default="scope">
          <image-preview :src="scope.row.repairImage" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="处理照片" align="center" prop="handleImage" width="100">
        <template #default="scope">
          <image-preview :src="scope.row.handleImage" :width="50" :height="50"/>
        </template>
      </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="处理照片" prop="handleImage">
              <image-upload v-model="form.handleImage"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
const data = reactive({
  ...
  rules: {
    repairType: [
      { required: true, message: "报修类型 不能为空", trigger: "change" }
    ],
    repairTitle: [
    ...
    repairStatus: [
      { required: true, message: "报修状态 不能为空", trigger: "change" }
    ],
  }
})
